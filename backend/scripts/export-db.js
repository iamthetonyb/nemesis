...
function resolveRequestedFormat(args) {
  ...
  if (!format) {
    throw new Error('Missing value for "--format". Example: pnpm db:export -- --format sql');
  }
...
function resolveOutputPath(args, requestedFormat) {
  ...
  if (outArgIndex !== -1) {
    ...
    if (!fromArg) {
      throw new Error(
        'Missing value for "--out". Example: pnpm db:export -- --out data/exports/my-db.sqlite or data/exports/my-db.sql'
      );
    }
...
async function main() {
  ...
  if (!fs.existsSync(sourcePath)) {
    throw new Error(`Database source not found at ${sourcePath}. Seed first with \"pnpm db:reset\".`);
  }
  ...
  console.log(`Restore command: pnpm db:import -- --in \"${outputPath}\"`);
}
