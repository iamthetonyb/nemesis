...
const ALLOWED_SCOPE_TABLES = {
  package_regions: "package_regions.region_key",
  package_provinces: "package_provinces.province_key",
};

function queryPackagesPage(db, scopeTable, scopeColumn, scopeKey, normalizedQuery, options = {}) {
  if (!ALLOWED_SCOPE_TABLES[scopeTable] || ALLOWED_SCOPE_TABLES[scopeTable] !== scopeColumn) {
    throw new Error(`Invalid scope table/column: ${scopeTable}/${scopeColumn}`);
  }
