...
if (!hasSchema) {
  ...
  if (latestExport) {
    ...
    console.error(`Import the latest export with: pnpm db:import -- --in "${latestExport}"`);
  }

  console.error(`Run "pnpm db:reset" inside backend/ if you want to rebuild the database from seed data.`);
  db.close();
  process.exit(1);
}
