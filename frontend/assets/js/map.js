window.AuditMap = (() => {
  ...
  function renderMarkers(geo) {
    clearMarkers();

    if (!map || !geo || !Array.isArray(geo.features) || geo.features.length > 200) {
      return;
    }
  ...
  function addLayers() {
    ...
    map.addLayer({
      ...
      paint: {
        "fill-color": ["coalesce", ["get", "fillColor"], "#17130a"],
    ...
    map.addLayer({
      ...
      filter: ["==", ["get", "interactive"], true],
      layout: {
        "line-sort-key": ["coalesce", ["get", "sortKey"], 0],
      },
      paint: {
        "line-color": ["coalesce", ["get", "strokeColor"], "#d4af37"],
    ...
    map.addLayer({
      ...
      filter: ["==", ["get", "interactive"], true],
      paint: {
        "fill-color": ["coalesce", ["get", "fillColor"], "#d4af37"],
        "fill-opacity": [
          "case",
  ...
  function refresh(geo, getFeatureStyle) {
    ...
    clearHover();
    const styledGeo = buildStyledGeo(geo, getFeatureStyle);
    map.getSource(SOURCE).setData(styledGeo);
    renderMarkers(styledGeo);
  }
